...
export const AdminBookings: React.FC = () => {
  ...
  const [modalTab, setModalTab] = useState<"OVERVIEW" | "SERVICES" | "INVOICE">(
    "OVERVIEW",
  );
  ...
  const [newService, setNewService] = useState<Partial<ServiceCharge>>({
    serviceType: "MINIBAR",
    quantity: 1,
    amount: 0,
    description: "",
  ...
  const getStatusBadge = (status: BookingStatus) => {
    switch (status) {
      case BookingStatus.PENDING:
        return (
          <span className="px-2.5 py-1 bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <Info size={12} /> Chờ duyệt
          </span>
        );
      case BookingStatus.CONFIRMED:
        return (
          <span className="px-2.5 py-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <CheckCircle size={12} /> Đã xác nhận
          </span>
        );
      case BookingStatus.CHECKED_IN:
        return (
          <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <LogIn size={12} /> Đang ở
          </span>
        );
      case BookingStatus.CHECKED_OUT:
        return (
          <span className="px-2.5 py-1 bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <LogOut size={12} /> Đã trả
          </span>
        );
      case BookingStatus.COMPLETED:
        return (
          <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <CheckCircle size={12} /> Hoàn thành
          </span>
        );
      case BookingStatus.CANCELLED:
        return (
          <span className="px-2.5 py-1 bg-rose-50 text-rose-700 border border-rose-200 rounded-lg text-xs font-bold flex items-center w-fit gap-1">
            <XCircle size={12} /> Đã hủy
          </span>
        );
      default:
        return (
          <span className="px-2.5 py-1 bg-gray-50 text-gray-600 border border-gray-200 rounded-lg text-xs font-bold">
            {status}
          </span>
        );
    }
  ...
  const getPaymentBadge = (status: PaymentStatus) => {
    switch (status) {
      ...
      case PaymentStatus.REFUNDED:
        return (
          <span className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-1 rounded border border-purple-100 uppercase tracking-wide">
            Đã hoàn tiền
          </span>
        );
      default:
        return (
          <span className="text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded border border-rose-100 uppercase tracking-wide">
            Chưa thanh toán
          </span>
        );
    }
          ...
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-slate-500 font-medium">
              <span className="flex items-center gap-1">
                <Activity size={16} className="text-indigo-500" /> Hôm nay:
              </span>
              <span className="flex items-center gap-1">
                <LogIn size={14} className="text-emerald-500" />{" "}
                {stats?.todayCheckIns || 0} Check-in
              </span>
              <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
              <span className="flex items-center gap-1">
                <LogOut size={14} className="text-orange-500" />{" "}
                {stats?.todayCheckOuts || 0} Check-out
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-slate-900">
                {stats?.paidRevenue?.toLocaleString() || 0}đ
              </p>
            </div>
          </div>
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex items-center gap-3">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row gap-2 p-1">
            <select
              ...
              className="h-[42px] bg-slate-50 hover:bg-slate-100 rounded-xl px-4 text-xs font-bold text-slate-600 outline-none cursor-pointer border border-transparent focus:border-indigo-200"
            >
              <option value="All">Tất cả trạng thái</option>
              {Object.values(BookingStatus).map((s) => (
                <option key={s} value={s}>
                  {BookingStatusDisplay[s]}
                </option>
              ))}
            </select>
            ...
            >
              ...
              {Object.values(PaymentStatus).map((s) => (
                <option key={s} value={s}>
                  {PaymentStatusDisplay[s]}
                </option>
              ))}
            </select>

            <div
              className={`flex items-center gap-2 bg-slate-50 rounded-xl px-2 transition-all ${!useDateFilter ? "opacity-50" : ""}`}
            >
              <input
                type="date"
                value={startDate}
                onChange={(e) => setStartDate(e.target.value)}
                className="bg-transparent text-xs font-bold text-slate-600 outline-none w-24 py-2.5"
              />
            ...
            <button
              ...
              className={`p-2.5 rounded-xl transition-colors ${useDateFilter ? "bg-indigo-100 text-indigo-600" : "bg-slate-100 text-slate-400"}`}
            >
            ...
            <button
              type="submit"
              className="h-[42px] px-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all flex items-center gap-2"
            >
              Lọc
            </button>
          </div>
            ...
            <div className="flex-1 overflow-x-auto">
              <table className="w-full">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider pl-8">
                      Booking ID
                    </th>
                    ...
                    bookings.map((booking) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap align-middle pl-8">
                          <span className="block text-sm font-bold text-indigo-600 font-mono group-hover:text-indigo-700 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-slate-500 flex items-center gap-1 font-medium">
                                <Phone size={10} /> {booking.userPhoneNumber}
                              </div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap align-middle">
                          <div className="text-sm font-bold text-slate-900 font-mono mb-1">
                        ...
                        <td
                          className="px-6 py-4 whitespace-nowrap text-right align-middle pr-8"
                          ...
                          <div className="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100">
                            {booking.status === BookingStatus.PENDING && (
                              <>
                                <button
                                  onClick={(e) =>
                                    handleAction(booking.id, "confirm", e)
                                  }
                                  disabled={!!actionLoading}
                                  className="px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-xs font-bold shadow-md shadow-emerald-200 hover:bg-emerald-600 transition-all flex items-center gap-1"
                                >
                            ...
                            {booking.status === BookingStatus.CHECKED_IN && (
                              <button
                                onClick={(e) =>
                                  handleAction(booking.id, "check-out", e)
                                }
                                disabled={!!actionLoading}
                                className="px-3 py-1.5 bg-orange-500 text-white rounded-lg text-xs font-bold shadow-md shadow-orange-200 hover:bg-orange-600 transition-all flex items-center gap-1.5"
                              >
                                {actionLoading === booking.id ? (
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-sm mt-1">
                            Thử thay đổi bộ lọc ngày hoặc trạng thái.
                          </p>
                          <button
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh] animate-scale-in">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium mt-0.5">
                    {new Date(selectedBooking.createdDate).toLocaleString(
                      "vi-VN",
                    )}
                  </p>
                </div>
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        <div className="flex justify-between border-b border-slate-50 pb-2">
                          ...
                          <span className="font-bold text-slate-900">
                            {selectedBooking.userPhoneNumber}
                          </span>
                        </div>
                        <div className="flex justify-between border-b border-slate-50 pb-2">
                          <span className="text-slate-500 font-medium">
                        ...
                        <div className="flex justify-between border-b border-slate-50 pb-2">
                          ...
                          <span className="font-bold text-slate-900">
                            {selectedBooking.primaryGuest?.cccdNumber || "N/A"}
                          </span>
                        </div>
                        <div className="flex justify-between pt-1">
                          ...
                          <span className="font-bold text-slate-900">
                            {selectedBooking.numberOfGuests} lớn,{" "}
                            {selectedBooking.numberOfChildren} trẻ
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="animate-fade-in space-y-6">
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                    <h5 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2">
                      <Plus size={16} className="text-indigo-600" /> Thêm dịch
                      vụ mới
                    </h5>
                    <form
                    ...
                    >
                      ...
                      <div className="col-span-8 md:col-span-5">
                        ...
                        <input
                          type="text"
                          placeholder="Ví dụ: Nước ngọt, Mì ly..."
                      ...
                      <div className="col-span-6 md:col-span-12 flex justify-end items-end">
                        <button
                          ...
                          className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold text-sm transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-200"
                        >
                          {addingService ? (
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="space-y-4">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-slate-500 font-medium">
                          Giá phòng ({selectedBooking.numberOfNights} đêm)
                        </span>
                        <span className="font-bold text-slate-900">
                          {(
                            (selectedBooking.roomPricePerNight || 0) *
                            (selectedBooking.numberOfNights || 1)
                          ).toLocaleString()}
                          <span className="text-xs ml-1">VND</span>
                        </span>
                      </div>
                      ...
                      <div className="border-t border-slate-100 pt-4 mt-2">
                        <div className="flex justify-between items-end">
                          ...
                          <span className="text-3xl font-bold text-indigo-600">
                            ...
                            <span className="text-xl text-indigo-600 ml-2">
                              VND
                            </span>
                          </span>
                        </div>
            ...
            <div className="p-6 border-t border-slate-100 bg-white flex justify-end gap-3 z-10 relative">
              {selectedBooking.status === BookingStatus.PENDING && (
                <>
                  <button
                    ...
                    className="px-5 py-2.5 border border-rose-100 bg-rose-50 text-rose-600 font-bold rounded-xl hover:bg-rose-100 transition-colors"
                  >
                    Từ chối
                  </button>
                  <button
                    onClick={() => handleAction(selectedBooking.id, "confirm")}
                    className="px-6 py-2.5 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-200"
                  >
                    Duyệt Booking
                  </button>
                </>
              ...
              {selectedBooking.status === BookingStatus.CONFIRMED && (
                <button
                  onClick={() => handleAction(selectedBooking.id, "check-in")}
                  className="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200 flex items-center gap-2"
                >
              ...
              {selectedBooking.status === BookingStatus.CHECKED_IN && (
                <button
                  onClick={() => handleAction(selectedBooking.id, "check-out")}
                  className="px-6 py-2.5 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-colors shadow-lg shadow-orange-200 flex items-center gap-2"
                >
            ...
            <div className="p-6 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500 mt-1">
                ...
                <span className="font-mono font-bold text-indigo-600">
                  {
                    bookings.find((b) => b.id === checkInBookingId)
                      ?.bookingNumber
                  }
                </span>
              </p>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCheckInData({
                      ...checkInData,
                      depositTransactionId: e.target.value,
                    })
                  }

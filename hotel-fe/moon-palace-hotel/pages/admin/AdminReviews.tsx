...
import { MessageSquare, Star, CheckCircle, EyeOff, Reply, ExternalLink, Filter, AlertCircle, BedDouble, ChevronDown, Loader2, RefreshCw } from 'lucide-react';
...
export const AdminReviews: React.FC = () => {
    ...
    try {
      ...
      for (const room of rooms) {
        try {
          ...
          if (roomDetail.data.status === 'success' && roomDetail.data.data.reviews) {
  ...
  const handleReplySubmit = async (reviewId: string) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        setReviews(prev => prev.map(r => 
          r.id === reviewId ? response.data.data : r
        ));
  ...
  const getRoomImage = (roomId: string) => {
      ...
      if (!room) return 'https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=1000&auto=format&fit=crop';
      return room.images?.[0] || room.thumbnailImage || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=1000&auto=format&fit=crop';
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-4">
                <button 
                  ...
                  title="Làm mới"
                >
                  <RefreshCw size={18} className={`text-lux-600 ${loading ? 'animate-spin' : ''}`}/>
                </button>
                ...
                <div className="flex items-center gap-4 bg-white p-3 rounded-xl border border-lux-200 shadow-sm">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-2xl font-serif font-bold text-lux-900">{reviews.length}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-lux-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
             <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                ...
                {[
                  ...
                  { value: 'REJECTED', label: 'Đã ẩn' }
                    ...
                    <button 
                        key={f.value}
                        onClick={() => {setFilterStatus(f.value as any); setCurrentPage(0);}}
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
                 ...
                 <div className="relative w-full md:w-64">
                     <select 
                        value={filterRoom}
                        onChange={(e) => {setFilterRoom(e.target.value); setCurrentPage(0);}}
          ...
          <div className="space-y-6">
              {filteredReviews.length > 0 ? (
                  filteredReviews.map(review => {
                    ...
                    return (
                      <div key={review.id} className="bg-white rounded-2xl border border-lux-200 shadow-sm relative transition-all hover:shadow-md overflow-hidden">
                          <div className="flex flex-col md:flex-row">
                              ...
                              <div className="bg-lux-50 p-6 md:w-64 flex-shrink-0 border-b md:border-b-0 md:border-r border-lux-100 flex flex-col gap-4">
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-lux-400 mt-1">Loại: {review.roomType}</p>
                                    </div>
                              </div>
                              ...
                              <div className="p-6 flex-1 flex flex-col">
                                  ...
                                  <div className="flex items-center gap-3 mb-4">
                                      ...
                                      />
                                      <div>
                                          <h4 className="font-bold text-lux-900 text-sm">{review.userFullName}</h4>
                                          <p className="text-xs text-lux-500">{formatDate(review.createdDate)}</p>
                                      </div>
                                       ...
                                       {review.replies && review.replies.length > 0 && (
                                         <div className="space-y-2 mb-4">
                                           {review.replies.map(reply => (
                                             <div key={reply.id} className="bg-lux-50 p-4 rounded-xl border border-lux-100 flex gap-3 animate-fade-in">
                                                 <div className="w-8 h-8 rounded-full bg-lux-900 flex items-center justify-center text-white shrink-0">
                                                     <Reply size={14}/>
                                                 </div>
                                                 <div className="flex-1">
                                                     <p className="text-xs font-bold text-lux-900 mb-1">{reply.userFullName}</p>
                                                     <p className="text-sm text-lux-600">{reply.content}</p>
                                       ...
                                       {openReplyId === review.id ? (
                                           <div className="mt-4 animate-fade-in bg-lux-50 p-4 rounded-xl border border-lux-200">
                                               ...
                                               <div className="flex gap-2 justify-end">
                                                   <button 
                                                      onClick={() => {setOpenReplyId(null); setReplyText({...replyText, [review.id]: ''});}}
                                  ...
                                  <div className="pt-4 mt-4 border-t border-lux-50 flex items-center justify-between">
                                      <div className="text-xs font-bold text-lux-400 uppercase tracking-wider">
                                          Trạng thái: <span className={getStatusColor(review.reviewStatus)}>{getStatusDisplay(review.reviewStatus)}</span>
                                      </div>
                                      <div className="flex gap-2">
                                          {review.reviewStatus === 'PENDING' && (
                                              <button 
                                                onClick={() => handleStatusChange(review.id, 'APPROVED' as ReviewStatus)}
                                                disabled={isProcessing}
                                              ...
                                              >
                                                  {isProcessing ? <Loader2 size={14} className="animate-spin"/> : <CheckCircle size={14}/>} Duyệt
                                              </button>
                                          )}
                                          {review.reviewStatus !== 'REJECTED' && (
                                              <button 
                                                onClick={() => handleStatusChange(review.id, 'REJECTED' as ReviewStatus)}
                                                disabled={isProcessing}
                                              ...
                                              >
                                                  {isProcessing ? <Loader2 size={14} className="animate-spin"/> : <EyeOff size={14}/>} Ẩn
                                              </button>
                                          )}
                                          {review.reviewStatus === 'REJECTED' && (
                                               <button 
                                              ...
                                              >
                                                  {isProcessing ? <Loader2 size={14} className="animate-spin"/> : <CheckCircle size={14}/>} Hiện lại
                                              </button>
                                          )}
                                      </div>
                                  </div>
                  ...
                  <div className="text-center py-20 bg-white rounded-2xl border border-lux-200">
                      ...
                      <button 
                        onClick={() => {setFilterStatus('All'); setFilterRoom('All'); setCurrentPage(0);}}

...
//khai báo list dịch vụ cho tạo phòng
...
export const AdminRooms: React.FC = () => {
  ...
  // Ảnh server hiện tại (khi EDIT) - track những ảnh còn giữ lại
  const [existingImages, setExistingImages] = useState<string[]>([]);
  const [existingThumbnailIndex, setExistingThumbnailIndex] =
    useState<number>(0);
  ...
  // Compress ảnh xuống < 4MB trước khi upload
  ...
  const processFiles = async (files: FileList | File[]) => {
    ...
    compressedFiles.forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        loaded++;
        if (loaded === compressedFiles.length) {
  ...
  const openEditModal = (room: Room) => {
    ...
    setThumbnailFile(null);
    setThumbnailIndex(0);
    // Load ảnh server hiện tại vào state
    const imgs =
    ...
    try {
      if (modalMode === "CREATE") {
        const createPayload: CreateRoomRequest = {
          ...
          thumbnailImage:
            imageFiles.length > 0 ? imageFiles[thumbnailIndex] : undefined,
        };
        ...
        const updatePayload: any = {
          ...
          hasKitchen: formData.hasKitchen,
          notes: formData.notes,
          // Danh sách URL ảnh cũ còn giữ lại
          keepImageUrls: existingImages,
        };
        // Nếu có upload ảnh mới thì thêm vào
        if (imageFiles.length > 0) {
          updatePayload.images = imageFiles;
          updatePayload.thumbnailImage = imageFiles[thumbnailIndex];
        }
      ...
      const serverMsg =
        err.response?.data?.message || "Có lỗi xảy ra khi lưu phòng.";
  ...
  const handleToggleFeatured = async (room: Room) => {
    ...
    const updatedRooms = rooms.map((r) =>
      r.id === room.id ? { ...r, isFeatured: !r.isFeatured } : r,
    );
    ...
    try {
      ...
      showNotification(
        room.isFeatured ? "Đã bỏ nổi bật" : "Đã đánh dấu nổi bật",
        "success",
      );
    } catch (error) {
      // Rollback
  ...
  const getStatusBadge = (status: RoomStatus) => {
    switch (status) {
      case RoomStatus.AVAILABLE:
        return (
          <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-lg text-xs font-bold flex items-center gap-1">
            <CheckCircle size={12} /> Trống
          </span>
        );
      case RoomStatus.OCCUPIED:
        return (
          <span className="px-2.5 py-1 bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-bold flex items-center gap-1">
            <ShieldAlert size={12} /> Đã có khách
          </span>
        );
      case RoomStatus.MAINTENANCE:
        return (
          <span className="px-2.5 py-1 bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-xs font-bold flex items-center gap-1">
            <RotateCcw size={12} /> Bảo trì
          </span>
        );
      case RoomStatus.CLEANING:
        return (
          <span className="px-2.5 py-1 bg-cyan-100 text-cyan-700 border border-cyan-200 rounded-lg text-xs font-bold flex items-center gap-1">
            <Wind size={12} /> Dọn dẹp
          </span>
        );
      case RoomStatus.RESERVED:
        return (
          <span className="px-2.5 py-1 bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-xs font-bold flex items-center gap-1">
            <ShieldAlert size={12} /> Đặt trước
          </span>
        );
      ...
      default:
        return (
          <span className="px-2.5 py-1 bg-gray-100 text-gray-700 border border-gray-200 rounded-lg text-xs font-bold">
            {status}
          </span>
        );
    }
    ...
    <div className="max-w-[1600px] mx-auto p-6 font-sans text-slate-800 animate-fade-in">
      ...
      <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Danh sách toàn bộ phòng nghỉ trong hệ thống.
          </p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-b-2xl rounded-tr-2xl shadow-sm border border-slate-200 border-t-0 mb-6 flex flex-col xl:flex-row items-center gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
          ...
          {/* Status Filter */}
          <div className="flex items-center gap-2 w-full md:w-auto">
            <span className="text-slate-400 font-bold text-xs shrink-0 px-2">
              Trạng thái:
            </span>
            <select
              value={filterStatus}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1 text-xs text-slate-600">
                          <span className="mb-1 font-medium">
                            {room.maxOccupancy} Khách • {room.size}m²
                          </span>
                          <div className="flex gap-1 flex-wrap">
                            {room.amenities.slice(0, 2).map((a, i) => (
                              <span
                                key={i}
                                className="bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded text-[10px] whitespace-nowrap"
                              >
                                {a}
                              </span>
                            ))}
                      ...
                      <td className="px-6 py-4 text-center">
                        <button
                          ...
                          title={
                            room.isFeatured ? "Bỏ nổi bật" : "Đánh dấu nổi bật"
                          }
                        >
                          <Star
                            size={18}
                            fill={room.isFeatured ? "currentColor" : "none"}
                          />
                        </button>
                      </td>
          ...
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl relative z-10 overflow-hidden max-h-[90vh] flex flex-col animate-scale-up">
            ...
            <div className="flex-1 overflow-y-auto p-8 bg-white">
              {modalMode === "VIEW" && viewingRoom ? (
                    ...
                    <div className="w-full md:w-2/3 space-y-6">
                      <div>
                        ...
                        <div className="text-indigo-600 font-bold text-lg mt-1">
                          {viewingRoom.pricePerNight.toLocaleString()} VNĐ{" "}
                      ...
                      <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                        ...
                        <div>
                          ...
                          <span className="font-medium">
                            {viewingRoom.bedCount}x {viewingRoom.bedType}
                          </span>
                        </div>
                        <div>
                          <span className="text-slate-500 text-xs uppercase font-bold">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900 mb-2 flex items-center gap-2">
                          <Wifi size={16} /> Tiện nghi
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {viewingRoom.amenities.map((a, i) => (
                            <span
                              key={i}
                              className="px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 font-medium text-sm border border-indigo-100"
                            >
                              {a}
                            </span>
                          ))}
                ...
                >
                      ...
                      {modalMode === "EDIT" && (
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            value={currentStatus}
                            onChange={(e) =>
                              setCurrentStatus(e.target.value as RoomStatus)
                            }
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            value={formData.type}
                            onChange={handleInputChange}
                            className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 cursor-pointer"
                          >
                            {Object.values(RoomType).map((t) => (
                              <option key={t} value={t}>
                                {RoomTypeDisplay[t]}
                    ...
                    <div className="flex flex-wrap gap-4 mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer hover:bg-slate-100 p-1 rounded transition-colors">
                        <input
                          type="checkbox"
                          name="allowSmoking"
                          checked={formData.allowSmoking || false}
                          onChange={handleInputChange}
                          className="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500"
                        />
                        <span className="text-sm font-medium text-slate-700">
                        ...
                        <div className="w-full p-2 bg-slate-50 border border-slate-200 rounded-xl focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20 transition-all">
                          <div className="flex flex-wrap gap-2 mb-2">
                            {formData.amenities.map((amenity, idx) => (
                              <span
                                key={idx}
                                className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-white border border-slate-200 text-sm font-medium text-slate-700 shadow-sm animate-scale-up"
                              >
                                {amenity}
                                <button
                                  type="button"
                            ...
                            <input
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && handleAddAmenity(e)
                              }
                  ...
                  {/* Section 4: Image Upload Manager */}
                  <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">
                      Hình ảnh phòng
                    </h4>
                    ...
                    >
                      <label className="flex flex-col items-center justify-center gap-3 py-10 cursor-pointer">
                        <div
                          className={`p-4 rounded-2xl transition-colors ${isDragging ? "bg-indigo-100 text-indigo-500" : "bg-white text-slate-400 border border-slate-200 shadow-sm"}`}
                        >
                          <Upload size={28} />
                        </div>
                        ...
                        <span className="px-5 py-2 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200 flex items-center gap-2">
                    ...
                    {/* Edit mode: existing images from server - có thể xóa từng ảnh */}
                      ...
                      <div className="mt-5">
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-xs font-bold text-slate-600 flex items-center gap-2">
                            ...
                            <span className="text-slate-400 font-normal">
                              • Nhấn để đặt đại diện · Hover để xóa
                            </span>
                          </p>
                          <button
                            type="button"
                            onClick={() => setExistingImages([])}
                            className="text-xs text-red-500 hover:text-red-700 font-bold flex items-center gap-1 px-2 py-1 hover:bg-red-50 rounded-lg transition-colors"
                          >
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                          {existingImages.map((img, idx) => (
                            <div
                              key={idx}
                              onClick={() => setExistingThumbnailIndex(idx)}
                              className={`relative aspect-square rounded-xl overflow-hidden border-2 cursor-pointer transition-all duration-150 group ${
                                existingThumbnailIndex === idx &&
                                imagePreviews.length === 0
                                  ? "border-indigo-500 ring-2 ring-indigo-500/25 shadow-lg shadow-indigo-100"
                            ...
                            >
                              ...
                              {existingThumbnailIndex === idx &&
                                imagePreviews.length === 0 && (
                                  <div className="absolute top-1.5 left-1.5 bg-indigo-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-md flex items-center gap-0.5">
                                    <Star size={8} fill="white" /> ĐẠI DIỆN
                                  </div>
                                )}
                              {/* Remove button */}
                              <button
                                ...
                                onClick={(e) => {
                                  ...
                                  if (existingThumbnailIndex === idx)
                                    setExistingThumbnailIndex(0);
                                  else if (existingThumbnailIndex > idx)
                                    setExistingThumbnailIndex(
                                      (prev) => prev - 1,
                                    );
                                }}
                              ...
                              <div className="absolute bottom-1 right-1.5 text-[9px] font-bold text-white/80 bg-black/40 rounded px-1">
                                {idx + 1}
                              </div>
                            </div>
                          ))}
                        ...
                        {imagePreviews.length === 0 && (
                          <div className="mt-3 p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-2 text-slate-600 text-xs font-medium">
                            <Star
                              size={13}
                              fill="currentColor"
                            ...
                            <span className="font-bold">
                              Ảnh số {existingThumbnailIndex + 1}
                            </span>
                            <span className="text-slate-400 ml-auto">
                              Upload ảnh mới bên dưới để thêm vào
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    {imagePreviews.length > 0 && (
                      <div className="mt-5">
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                          ...
                          {/* Add more button */}
                          <label className="aspect-square rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-300 hover:bg-indigo-50/40 transition-all text-slate-400 hover:text-indigo-500">
                            <Plus size={20} />
                        ...
                        {/* Info bar */}
                        <div className="mt-3 p-3 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center gap-2 text-indigo-700 text-xs font-medium">
                          <Star
                            size={13}
                            fill="currentColor"
                            className="text-indigo-400 shrink-0"
                          ...
                          Ảnh đại diện:{" "}
                          <span className="font-bold">
                            Ảnh số {thumbnailIndex + 1}
                          </span>
                          <span className="text-indigo-400 ml-auto">
                            Nhấn vào ảnh để chọn đại diện
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {imagePreviews.length === 0 &&
                      existingImages.length === 0 && (
                        <div className="mt-4 flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-xl text-amber-700 text-xs">
                          <AlertTriangle size={14} className="shrink-0" />
                          <span>
                            {modalMode === "EDIT"
                              ? "Đã xóa hết ảnh cũ. Upload ảnh mới bên trên để thay thế."
                              : "Chưa có ảnh nào. Phòng sẽ hiển thị ảnh mặc định."}
                          </span>
                        </div>
                      )}
                  </div>
            ...
            {modalMode !== "VIEW" && (
              <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="px-6 py-3 border border-slate-200 bg-white rounded-xl font-bold text-slate-700 hover:bg-slate-100 transition-colors"
                >
                  Hủy bỏ
                </button>
                <button

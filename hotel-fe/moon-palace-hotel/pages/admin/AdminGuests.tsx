...
export const AdminGuests: React.FC = () => {
  ...
  const handleToggleLock = async (user: User, e: React.MouseEvent) => {
    ...
    setUsers((currentUsers) =>
      currentUsers.map((u) =>
        u.id === user.id ? { ...u, isLocked: newIsLocked } : u,
      ),
    );
  ...
  const handleToggleActive = async (user: User, e: React.MouseEvent) => {
    ...
    if (user.isLocked) {
      showNotification(
        "Vui lòng mở khóa tài khoản trước khi kích hoạt",
        "error",
      );
      return;
    }
  ...
  const handleDeleteUser = async (user: User, e: React.MouseEvent) => {
    ...
    if (
      !window.confirm(
        `CẢNH BÁO: Bạn có chắc chắn muốn xóa người dùng "${user.fullName}"? Hành động này không thể hoàn tác.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      showNotification(
        "Không thể xóa người dùng này (có thể do ràng buộc dữ liệu)",
        "error",
      );
    }
  };
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Hệ thống quản lý tài khoản, phân quyền và bảo mật.
            </p>
          </div>
          <button
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-[40%] pl-8">
                      Thành viên
                    </th>
                    ...
                    users.map((user) => (
                      ...
                      >
                        <td className="px-6 py-4 pl-8">
                          <div
                            className={`flex items-center gap-4 transition-all duration-300 ${user.isLocked ? "opacity-80" : "opacity-100"}`}
                          >
                            <div className="relative">
                              <div
                                className={`w-11 h-11 rounded-xl flex items-center justify-center border shrink-0 uppercase font-bold text-lg overflow-hidden shadow-sm transition-all
                                                            ${
                                                              user.isLocked
                              ...
                              {user.isLocked && (
                                <div className="absolute inset-0 bg-slate-900/10 backdrop-blur-[1px] rounded-xl flex items-center justify-center">
                                  <Lock
                                    size={16}
                                    className="text-rose-600 drop-shadow-sm"
                                  />
                                </div>
                              )}
                            </div>
                            <div>
                              <div
                                className={`font-bold text-[15px] flex items-center gap-2 ${user.isLocked ? "text-slate-500" : "text-slate-900"}`}
                              >
                                {user.fullName}
                              </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap align-middle">
                          <span
                            className={`px-3 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wider border ${user.isLocked ? "bg-slate-100 text-slate-400 border-slate-200" : getRoleBadge(user.role)}`}
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center align-middle">
                          ...
                          >
                            {user.isLocked ? (
                              ...
                              >
                                <span
                                  className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-300 ${user.isActive ? "translate-x-5.5 left-0" : "translate-x-0.5 left-0"}`}
                                />
                              </button>
                            )}
                          </div>
          ...
          <div className="relative bg-gradient-to-br from-indigo-600 to-purple-700 p-8 text-white overflow-hidden shrink-0">
            ...
            <div className="grid grid-cols-3 gap-3 mt-8 relative z-10">
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 text-center border border-white/5">
                ...
                <span className="text-[10px] text-indigo-100 font-bold uppercase tracking-wider opacity-80">
                  Bookings
                </span>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 text-center border border-white/5">
          ...
          <div className="flex-1 overflow-y-auto bg-slate-50 p-6 relative custom-scrollbar">
            {historyLoading ? (
                  ...
                  {historyBookings.map((booking, idx) => {
                    ...
                    return (
                      ...
                      >
                        {/* Timeline Dot */}
                        <div
                          className={`absolute left-[13px] top-6 w-3.5 h-3.5 rounded-full border-[3px] border-white shadow-sm z-10 ring-4 ring-slate-50 transition-all group-hover:scale-110 ${styles.dot}`}
                        ></div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                          <div className="p-5">
                            <div className="flex justify-between items-start mb-3">
                              ...
                              <span
                                className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide border ${styles.bg} ${styles.text} ${styles.border}`}
                              >
                                {booking.status}
                              </span>
                            </div>
              ...
              <div className="flex flex-col items-center justify-center py-32 text-slate-300">
                ...
                <p className="text-sm text-slate-400 mt-2 max-w-[200px] text-center">
                  Khách hàng này chưa thực hiện bất kỳ giao dịch đặt phòng nào.
                </p>
              </div>
            )}
          </div>

...
import { ArrowLeft, Star, Users, Maximize, CalendarCheck, Phone, CheckCircle2, Wifi, Coffee, Monitor, Wind, ChevronRight, Sun, Award, ShieldCheck, MessageSquare, Reply as ReplyIcon, Utensils, Flower2, X, ChevronLeft, Loader2, AlertCircle, Edit2, Trash2, Mail, MapPin } from 'lucide-react';
...
export const RoomDetail: React.FC = () => {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        setReviewSuccess('Đánh giá của bạn đã được gửi thành công và đang chờ duyệt!');
        setUserComment('');
        setUserRating(5);
  ...
  const handleEditReply = async (replyId: string) => {
    ...
    try {
      const response = await reviewApi.updateReply(replyId, editReplyContent.trim());
      if (response.data.status === 'success') {
      ...
      alert(error.response?.data?.message || 'Có lỗi xảy ra khi cập nhật phản hồi!');
    }
  };
  ...
  const fetchRoomData = async () => {
    ...
    try {
      ...
      if (response.data.status === "success" && response.data.data.reviews) {
        setReviews(response.data.data.reviews.filter((r: any) => r.reviewStatus === 'APPROVED'));
      }
    } catch (error) {
  ...
  const handleUserHover = (userId: string, userName: string, event: React.MouseEvent) => {
  ...
  return (
    <div className="min-h-screen bg-lux-50 pb-20 animate-fade-in font-sans selection:bg-lux-200 pt-24">
              ...
              return (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    <img 
                      src={user.userAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.userFullName)}&background=D4AF37&color=fff`}
                      alt={user.userFullName}
                      className="w-12 h-12 rounded-full border-2 border-lux-200 shadow-sm"
                    />
                    <div>
                      <h4 className="font-bold text-lux-900">{user.userFullName}</h4>
      ...
      {/* Lightbox Modal */}
      {lightboxOpen && (
          <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center backdrop-blur-sm animate-fade-in" onClick={() => setLightboxOpen(false)}>
              <button onClick={() => setLightboxOpen(false)} className="absolute top-6 right-6 text-white/70 hover:text-white p-2 rounded-full hover:bg-white/10 transition-all transform hover:scale-110">
                  <X size={32}/>
              </button>
              
              <button onClick={handlePrevImage} className="absolute left-4 md:left-8 p-3 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all transform hover:scale-110">
                  <ChevronLeft size={40}/>
              </button>
              ...
              <button onClick={handleNextImage} className="absolute right-4 md:right-8 p-3 text-white/70 hover:text-white hover:bg-white/10 rounded-full transition-all transform hover:scale-110">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-lux-100 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     ...
                     <div className="flex flex-col items-center text-center p-4 bg-lux-50 rounded-2xl hover:bg-lux-100 transition-all transform hover:scale-105 cursor-default">
                         <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-lux-500 shadow-sm mb-3 animate-bounce-slow" style={{ animationDelay: '0.1s' }}>
                             <Award size={24}/>
                         </div>
                     ...
                     <div className="flex flex-col items-center text-center p-4 bg-lux-50 rounded-2xl hover:bg-lux-100 transition-all transform hover:scale-105 cursor-default">
                         <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-lux-500 shadow-sm mb-3 animate-bounce-slow" style={{ animationDelay: '0.2s' }}>
                             <ShieldCheck size={24}/>
                         </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl border border-lux-100 shadow-sm hover:shadow-md transition-shadow">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                     {room.amenities.map((amenity, idx) => (
                         <div key={idx} className="flex items-center gap-4 p-3 hover:bg-lux-50 rounded-xl transition-all transform hover:scale-105 group cursor-default">
             ...
             <div className="bg-white p-6 md:p-8 rounded-3xl border border-lux-100 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-8">
                     <div>
                         ...
                         <div className="flex items-center gap-2">
                             ...
                             <span className="font-bold text-lux-900 text-lg">{room.averageRating?.toFixed(1) || '5.0'}</span>
                ...
                {reviewError && (
                  <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3 animate-fade-in">
                    ...
                    <p className="text-sm text-red-800 font-medium">{reviewError}</p>
                  </div>
                )}

                {/* Review Form */}
                {showReviewForm && (
                    <form onSubmit={handleSubmitReview} className="mb-8 bg-lux-50 p-6 rounded-2xl border border-lux-100 animate-slide-down">
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex gap-2">
                                {[1,2,3,4,5].map(star => (
                                    <button type="button" key={star} onClick={() => setUserRating(star)} className="focus:outline-none hover:scale-125 transition-transform">
                                        <Star size={24} className={`${star <= userRating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 fill-gray-300'} transition-all`}/>
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                            <button type="button" onClick={() => {setShowReviewForm(false); setReviewError(null);}} className="px-4 py-2 text-lux-600 font-bold text-sm hover:bg-lux-100 rounded-lg transition-all">Hủy</button>
                            <button disabled={isSubmittingReview} type="submit" className="px-6 py-2 bg-lux-900 text-white rounded-xl font-bold text-sm hover:bg-lux-800 disabled:opacity-70 flex items-center gap-2 transition-all transform hover:scale-105">
                ...
                {reviewsLoading ? (
                  ...
                  <div className="space-y-6">
                      {reviews.length > 0 ? reviews.map((review, idx) => (
                          <div key={review.id} className="border-b border-lux-50 last:border-0 pb-6 last:pb-0 animate-fade-in-up hover:bg-lux-50 p-4 rounded-xl transition-all" style={{ animationDelay: `${idx * 0.1}s` }}>
                              <div className="flex gap-4">
                                  <div 
                                    className="relative cursor-pointer group"
                                    onMouseEnter={(e) => handleUserHover(review.userId, review.userFullName, e)}
                                    onMouseLeave={() => setHoveredUserId(null)}
                                  >
                                    <img 
                                      src={review?.userAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(review.userFullName)}&background=D4AF37&color=fff`} 
                                  ...
                                  <div className="flex-1">
                                      <div className="flex justify-between items-start mb-1">
                                          <div>
                                            ...
                                            <span className="text-xs text-lux-400 ml-2">{formatDate(review.createdDate)}</span>
                                          </div>
                                      ...
                                      <div className="flex gap-0.5 mb-2">
                                          {Array.from({length: 5}).map((_, i) => (
                                              <Star key={i} size={12} className={`${i < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'} transition-all`}/>
                                          ))}
                                      </div>
                                      ...
                                      {editingReviewId === review.id ? (
                                        <div className="animate-fade-in bg-white p-3 rounded-xl border border-lux-200 mb-3">
                                          ...
                                          <div className="flex gap-2 justify-end">
                                            <button 
                                              onClick={() => {setEditingReviewId(null); setEditComment('');}}
                                      ...
                                      {review.replies && review.replies.length > 0 && (
                                        <div className="space-y-2 mt-3">
                                          {review.replies.map((reply: Reply) => (
                                            <div key={reply.id} className="bg-lux-50 p-3 rounded-xl border border-lux-100 flex gap-3 hover:bg-lux-100 transition-all group">
                                                <div className="w-6 h-6 rounded-full bg-lux-900 flex items-center justify-center text-white shrink-0 mt-0.5">
                                                    <ReplyIcon size={12}/>
                                                </div>
                                                <div className="flex-1">
                                                    <div className="flex justify-between items-start mb-1">
                                                      <p className="text-xs font-bold text-lux-900">{reply.userFullName}</p>
                                                    ...
                                                    {editingReplyId === reply.id ? (
                                                      ...
                                                      <>
                                                        <p className="text-xs text-lux-600 leading-relaxed">{reply.content}</p>
                                                        <span className="text-[10px] text-lux-400 mt-1 block">{formatDate(reply.createdDate)}</span>
                                                      </>
                                                    )}
                                                </div>
                                            </div>
                                      ...
                                      {activeReplyId === review.id ? (
                                        <div className="mt-3 animate-slide-down bg-lux-50 p-3 rounded-xl border border-lux-200">
                                            ...
                                            <div className="flex gap-2 justify-end">
                                                <button 
                                                  onClick={() => {setActiveReplyId(null); setReplyContent({...replyContent, [review.id]: ''});}}
                          ...
                          <div className="text-center py-8 animate-fade-in">
                            ...
                            <p className="text-lux-500">Chưa có đánh giá nào. Hãy là người đầu tiên trải nghiệm!</p>
                          </div>
                      )}
                  </div>
                )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-lg shadow-lux-200/50 border border-lux-100 top-24 hover:shadow-xl transition-shadow">
               ...
               <div className="mb-6 border-b border-lux-50 pb-6">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="font-bold text-lux-900">{room.averageRating?.toFixed(1) || '5.0'}</span>
                      <span className="text-xs text-lux-400">({room.totalReviews || reviews.length} đánh giá)</span>
                  </div>
               ...
               <button onClick={handleBooking} className="w-full bg-lux-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-lux-800 transition-all shadow-xl shadow-lux-900/20 flex items-center justify-center gap-2 group transform hover:scale-105 active:scale-95">
            ...
            <div className="bg-white p-6 rounded-3xl border border-lux-100 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="space-y-3">
                    <div onClick={() => navigate('/menu')} className="flex items-center gap-3 p-3 bg-lux-50 rounded-xl cursor-pointer hover:bg-lux-100 transition-all transform hover:scale-105 group">
                        ...
                        <ChevronRight size={16} className="ml-auto text-lux-300 group-hover:text-lux-500 group-hover:translate-x-1 transition-all"/>
                    </div>
                    <div onClick={() => navigate('/services')} className="flex items-center gap-3 p-3 bg-lux-50 rounded-xl cursor-pointer hover:bg-lux-100 transition-all transform hover:scale-105 group">
                        <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-lux-500 group-hover:text-lux-900 transition-all group-hover:rotate-12">
                            <Flower2 size={20}/>
                        </div>
                        <div>
        ...
        <div className="mt-20 pt-10 border-t border-lux-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {similarRooms.map((similar, idx) => (
                    <div key={similar.id} onClick={() => navigate(`/room/${similar.id}`)} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-lux-100 hover:shadow-xl transition-all cursor-pointer group transform hover:scale-105 animate-fade-in-up" style={{ animationDelay: `${idx * 0.1}s` }}>

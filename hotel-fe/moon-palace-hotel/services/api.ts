...
import { 
  ApiResponse, Page, RoomStatus, User, 
  ...
  ChatSession, UserFilterParams, ResetPasswordRequest, 
  CreateGroupBookingRequest, PaymentMethod,
  DailyReport, MonthlyReport, ServiceCharge, RoomType, RoomResponse,
  RoomSearchRequest, RoomStatistics, CreateReviewRequest, ReviewResponse,
  CreateReplyRequest, ReviewFilterParams,
  CreateRoomRequest,
  UpdateRoomRequest,
} from '../types';
...
export const authApi = {
  ...
  loginWithFacebook: (data: { token: string; provider: string; code?: string }) => 
...
export const roomApi = {
  ...
  getAll: (page = 0, size = 10, sortBy = 'createdDate', sortOrder = 'desc', includeReviews = false) => 
    api.get<ApiResponse<Page<RoomResponse>>>(`/rooms?page=${page}&size=${size}&sortBy=${sortBy}&sortOrder=${sortOrder}&includeReviews=${includeReviews}`),
  
  getById: (id: string) => 
    api.get<ApiResponse<RoomResponse>>(`/rooms/${id}`),
  ...
  getFeatured: () => 
    api.get<ApiResponse<RoomResponse[]>>('/rooms/featured'),
  
  getAvailable: () => 
    api.get<ApiResponse<RoomResponse[]>>('/rooms/available'),
  ...
  getByPriceRange: (minPrice: number, maxPrice: number) => 
    api.get<ApiResponse<RoomResponse[]>>(`/rooms/price-range?minPrice=${minPrice}&maxPrice=${maxPrice}`),
...
export const adminRoomApi = {
  ...
  update: (id: string, data: UpdateRoomRequest) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === 'images') {
        ...
        (value as string[]).forEach(url => formData.append('keepImageUrls', url));
  ...
  updateStatus: (id: string, status: RoomStatus) => 
    api.patch<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/status?status=${status}`),
  ...
  addImages: (id: string, files: File[]) => {
    ...
    return api.post<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/images`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
  },
  
  removeImage: (id: string, imageUrl: string) => 
    api.delete<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/images?imageUrl=${encodeURIComponent(imageUrl)}`),
  
  setThumbnail: (id: string, imageUrl: string) => 
    api.patch<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/thumbnail?imageUrl=${encodeURIComponent(imageUrl)}`),
  
  addAmenities: (id: string, amenities: string[]) => 
    api.post<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/amenities`, amenities),
  
  removeAmenity: (id: string, amenity: string) => 
    api.delete<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/amenities?amenity=${encodeURIComponent(amenity)}`),
  
  updatePrice: (id: string, price: number) => 
    api.patch<ApiResponse<RoomResponse>>(`/rooms/admin/${id}/price?price=${price}`),
...
export const reviewApi = {
  ...
  update: (id: string, comment: string) => 
    api.put<ApiResponse<ReviewResponse>>(`/reviews/${id}/update?comment=${encodeURIComponent(comment)}`),
  ...
  updateReply: (replyId: string, content: string) => 
    api.put<ApiResponse<ReviewResponse>>(`/reviews/${replyId}/update-reply?content=${encodeURIComponent(content)}`),
  ...
  updateStatus: (id: string, status: string) => 
    api.put<ApiResponse<ReviewResponse>>(`/reviews/update-status/${id}?status=${status}`),
};
